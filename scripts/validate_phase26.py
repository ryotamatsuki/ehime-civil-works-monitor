...
def main():
    ...
    roads = SEED.get('roads')
    sabo = SEED.get('sabo')
    if not isinstance(roads, list): roads = []; errors.append('roads must be array')
    if not isinstance(sabo, list): sabo = []; errors.append('sabo must be array')

    for i, row in enumerate(roads):
        ctx = f'roads[{i}]'
        if not isinstance(row, list) or len(row) != 7:
            errors.append(f'{ctx}: expected 7 fields'); continue
        ...
        seed_ids.append(pid); seed_names.append((pid, name))
    ...
    if len(seed_ids) != len(set(seed_ids)): errors.append('duplicate Phase 2.6 project id')
    overlap = base_ids & set(seed_ids)
    if overlap: errors.append('Phase 2.6 IDs already exist in base: ' + ', '.join(sorted(overlap)))

    seen_names = {}
    for pid, name in seed_names:
        key = norm(name)
        ...
        seen_names[key] = pid
        if 'うち強靭化' in name or '工事' == name[-2:]: errors.append(f'{pid}: work-package-like production name')
    ...
    union = base_ids | set(seed_ids)
    if len(union) < 100: errors.append(f'canonical union below benchmark: {len(union)}')
    if len(seed_ids) != 59: errors.append(f'expected audited production additions 59, got {len(seed_ids)}')
    ...
    print(f'Phase 2.6 validation OK: {len(base_ids)} base + {len(seed_ids)} new = {len(union)} canonical projects')
    print(f'  new road: {len(roads)} / new sabo: {len(sabo)}')
